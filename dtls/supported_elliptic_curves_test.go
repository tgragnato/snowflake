...
func TestSupportedEllipticCurves(t *testing.T) {
	...
	caAnalyzer.onWrite = func(in []byte) {
		...
		for i := range messages {
			...
			if h.Header.Type == handshake.TypeClientHello {
				...
				if err := clientHello.Unmarshal(msg); err != nil {
					t.Error(err)
					return
				}
	...
	if clientErr := <-clientErr; clientErr != nil {
		t.Fatalf("client error: %v", clientErr)
	}

	for i := range expectedCurves {
		if expectedCurves[i] != actualCurves[i] {
			t.Errorf("curves in SupportedEllipticCurves mismatch at index %d: expected %v, got %v", i, expectedCurves[i], actualCurves[i])
		}
	}
}
