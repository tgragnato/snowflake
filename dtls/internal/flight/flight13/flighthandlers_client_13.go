...
func flight13_1Parse(
	...
	seq, msgs, items, ok := cache.FullPullMapItems(state.HandshakeRecvSequence, state.CipherSuite,
		dtlsflight.HandshakeCachePullRule{Typ: handshake.TypeServerHello, Epoch: cfg.InitialEpoch, IsClient: false, Optional: true},
	)
	if !ok {
...
func flight13_3Parse(
	...
	seq, msgs, items, ok := flightCtx.cache.FullPullMapItems(
		...
		dtlsflight.HandshakeCachePullRule{Typ: handshake.TypeEncryptedExtensions, Epoch: flightCtx.cfg.InitialEpoch + 1, IsClient: false, Optional: false},
	)
	if !ok {
		return 0, nil, nil
	}
