...
func (c *ChaCha20Poly1305) Encrypt(pkt *recordlayer.RecordLayer, raw []byte) ([]byte, error) {
	...
	for i := range 8 {
		nonce[4+i] ^= byte(seq64 >> (56 - uint(i)*8))
	}
	...
	binary.BigEndian.PutUint16(result[pkt.Header.Size()-2:], uint16(len(encrypted)))
...
func (c *ChaCha20Poly1305) Decrypt(header recordlayer.Header, in []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %v", dtlserrors.ErrDecryptPacket, err)
	}
