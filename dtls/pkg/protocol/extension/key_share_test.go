// SPDX-FileCopyrightText: 2026 The Pion community <https://pion.ly>
// SPDX-License-Identifier: MIT

package extension

import (
	"errors"
	"reflect"
	"testing"

	dtlserrors "github.com/pion/dtls/v3/internal/errors"
	"github.com/pion/dtls/v3/pkg/crypto/elliptic"
	"golang.org/x/crypto/cryptobyte"
)

func TestKeyShare_Marshal_ClientHello_EmptyVector_RoundTrip(t *testing.T) {
	// No fields set -> treated as ClientHello with empty client_shares vector
	var ks KeyShare
	raw, err := ks.Marshal()
	if err != nil {
		t.Error(err)
	}

	// Expect: type, ext_len=2, vector_len=0
	typeValue := ks.TypeValue()
	expect := []byte{
		byte(uint16(typeValue) >> 8), // G115: high byte extraction from uint16 extension type in test fixture.
		byte(uint16(typeValue)),      // G115: low byte extraction from uint16 extension type in test fixture.
		0x00, 0x02, 0x00, 0x00,
	}
	if !reflect.DeepEqual(expect, raw) {
		t.Errorf("expected %v, got %v", expect, raw)
	}

	var parsed KeyShare
	err = parsed.Unmarshal(raw)
	if err != nil {
		t.Error(err)
	}
	if parsed.ServerShare != nil {
		t.Errorf("expected nil, got %v", parsed.ServerShare)
	}
	if parsed.SelectedGroup != nil {
		t.Errorf("expected nil, got %v", parsed.SelectedGroup)
	}
	if 0 != len(parsed.ClientShares) {
		t.Errorf("expected %v, got %v", 0, len(parsed.ClientShares))
	}
}

func TestKeyShare_Marshal_ClientHello_OK(t *testing.T) {
	// X25519 public key
	p1 := []byte{
		0xa8, 0xf1, 0xb7, 0x2d, 0x70, 0x7e, 0x58, 0xa4, 0x41, 0x73, 0x9e, 0x21,
		0x7b, 0x62, 0x1e, 0xd1, 0x4d, 0x11, 0x69, 0xa6, 0xbf, 0x72, 0x21, 0xec,
		0xaf, 0x76, 0xf3, 0x4e, 0xec, 0x48, 0x52, 0x1,
	}
	// P-256 uncompressed point
	p2 := []byte{
		0x4, 0xc5, 0xf5, 0xee, 0xf0, 0x1f, 0x4d, 0x53, 0xa6, 0x42, 0xd8, 0x3b,
		0x3a, 0x40, 0x48, 0x22, 0x49, 0x7f, 0x1a, 0xc7, 0xce, 0x66, 0x85, 0xfb,
		0xc9, 0x91, 0x9d, 0x7f, 0x72, 0xce, 0x88, 0x30, 0xc3, 0x3d, 0x5, 0x4c,
		0x5f, 0x7a, 0xaa, 0x9d, 0xcf, 0x3b, 0x44, 0x2a, 0xc0, 0xc9, 0x9f, 0x3,
		0x2b, 0x5, 0x5b, 0xbb, 0x9c, 0x5d, 0x7f, 0xf9, 0x24, 0x51, 0x4b, 0xab,
		0x36, 0xce, 0x46, 0x55, 0x7b,
	}

	ks := &KeyShare{
		ClientShares: []KeyShareEntry{
			{Group: elliptic.X25519, KeyExchange: p1},
			{Group: elliptic.P256, KeyExchange: p2},
		},
	}

	raw, err := ks.Marshal()
	if err != nil {
		t.Error(err)
	}

	expect := []byte{
		0x0, 0x33, // extension type
		0x0, 0x6b, // extension length
		0x0, 0x69, // vec length
		0x0, 0x1d, // X25519
		0x0, 0x20, // length 32
		0xa8, 0xf1, 0xb7, 0x2d, 0x70, 0x7e, 0x58, 0xa4, 0x41, 0x73, 0x9e, 0x21,
		0x7b, 0x62, 0x1e, 0xd1, 0x4d, 0x11, 0x69, 0xa6, 0xbf, 0x72, 0x21, 0xec,
		0xaf, 0x76, 0xf3, 0x4e, 0xec, 0x48, 0x52, 0x1,
		0x0, 0x17, // P-256
		0x0, 0x41, // length 65
		0x4, 0xc5, 0xf5, 0xee, 0xf0, 0x1f, 0x4d, 0x53, 0xa6, 0x42, 0xd8, 0x3b,
		0x3a, 0x40, 0x48, 0x22, 0x49, 0x7f, 0x1a, 0xc7, 0xce, 0x66, 0x85, 0xfb,
		0xc9, 0x91, 0x9d, 0x7f, 0x72, 0xce, 0x88, 0x30, 0xc3, 0x3d, 0x5, 0x4c,
		0x5f, 0x7a, 0xaa, 0x9d, 0xcf, 0x3b, 0x44, 0x2a, 0xc0, 0xc9, 0x9f, 0x3,
		0x2b, 0x5, 0x5b, 0xbb, 0x9c, 0x5d, 0x7f, 0xf9, 0x24, 0x51, 0x4b, 0xab,
		0x36, 0xce, 0x46, 0x55, 0x7b,
	}

	if !reflect.DeepEqual(expect, raw) {
		t.Errorf("expected %v, got %v", expect, raw)
	}
}

func TestKeyShare_Marshal_ClientHello_Errors(t *testing.T) {
	t.Run("duplicate group", func(t *testing.T) {
		ks := &KeyShare{
			ClientShares: []KeyShareEntry{
				{Group: elliptic.X25519, KeyExchange: []byte{1}},
				{Group: elliptic.X25519, KeyExchange: []byte{2}},
			},
		}

		_, err := ks.Marshal()
		if !errors.Is(err, dtlserrors.ErrDuplicateKeyShare) {
			t.Errorf("expected error %v, got %v", dtlserrors.ErrDuplicateKeyShare, err)
		}
	})

	t.Run("key len = 0", func(t *testing.T) {
		ks := &KeyShare{
			ClientShares: []KeyShareEntry{
				{Group: elliptic.X25519, KeyExchange: nil},
			},
		}

		_, err := ks.Marshal()
		if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
			t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
		}
	})

	t.Run("key len > 65535", func(t *testing.T) {
		ks := &KeyShare{
			ClientShares: []KeyShareEntry{
				{Group: elliptic.X25519, KeyExchange: make([]byte, 65536)},
			},
		}

		_, err := ks.Marshal()
		if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
			t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
		}
	})
}

func TestKeyShare_Marshal_ServerHello_OK(t *testing.T) {
	// X25519 public key
	p1 := []byte{
		0xa8, 0xf1, 0xb7, 0x2d, 0x70, 0x7e, 0x58, 0xa4, 0x41, 0x73,
		0x9e, 0x21, 0x7b, 0x62, 0x1e, 0xd1, 0x4d, 0x11, 0x69, 0xa6, 0xbf, 0x72,
		0x21, 0xec, 0xaf, 0x76, 0xf3, 0x4e, 0xec, 0x48, 0x52, 0x1,
	}
	okShare := KeyShareEntry{Group: elliptic.X25519, KeyExchange: p1}
	ks := &KeyShare{ServerShare: &okShare}

	raw, err := ks.Marshal()
	if err != nil {
		t.Error(err)
	}

	expect := []byte{
		0x0, 0x33, // extension type
		0x0, 0x24, // extension length
		0x0, 0x1d, // X25519
		0x0, 0x20, // length 32
		0xa8, 0xf1, 0xb7, 0x2d, 0x70, 0x7e, 0x58, 0xa4, 0x41, 0x73, 0x9e, 0x21,
		0x7b, 0x62, 0x1e, 0xd1, 0x4d, 0x11, 0x69, 0xa6, 0xbf, 0x72, 0x21, 0xec,
		0xaf, 0x76, 0xf3, 0x4e, 0xec, 0x48, 0x52, 0x1,
	}

	if !reflect.DeepEqual(expect, raw) {
		t.Errorf("expected %v, got %v", expect, raw)
	}
}

func TestKeyShare_Marshal_HelloRetryRequest(t *testing.T) {
	group := elliptic.P384
	ks := &KeyShare{SelectedGroup: &group}

	raw, err := ks.Marshal()
	if err != nil {
		t.Error(err)
	}

	expect := []byte{
		0x0, 0x33, // extension type
		0x0, 0x2, // extension length
		0x0, 0x18, // P-384
	}

	if !reflect.DeepEqual(expect, raw) {
		t.Errorf("expected %v, got %v", expect, raw)
	}
}

func TestKeyShare_Marshal_MultipleContexts_Error(t *testing.T) {
	group := elliptic.X25519
	ks := &KeyShare{
		ClientShares:  []KeyShareEntry{{Group: group, KeyExchange: []byte{1}}},
		ServerShare:   &KeyShareEntry{Group: group, KeyExchange: []byte{2}},
		SelectedGroup: &group,
	}

	_, err := ks.Marshal()
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}
}

func TestKeyShare_Unmarshal_ClientHello(t *testing.T) {
	raw := []byte{
		0x0, 0x33, // extension type
		0x0, 0x11, // extension length
		0x0, 0xf, // vec length
		0x0, 0x1d, // X25519
		0x0, 0x1, // group length
		0x41,
		0x11, 0xff, // Non-supported group
		0x0, 0x1, // group length
		0x42,
		0x0, 0x18, // P-384
		0x0, 0x1, // group length
		0x43,
	}

	var ks KeyShare
	err := ks.Unmarshal(raw)
	if err != nil {
		t.Error(err)
	}
	if ks.ServerShare != nil {
		t.Errorf("expected nil, got %v", ks.ServerShare)
	}
	if ks.SelectedGroup != nil {
		t.Errorf("expected nil, got %v", ks.SelectedGroup)
	}

	if 3 != len(ks.ClientShares) {
		t.Errorf("expected %v, got %v", 3, len(ks.ClientShares))
	}
	if elliptic.X25519 != ks.ClientShares[0].Group {
		t.Errorf("expected %v, got %v", elliptic.X25519, ks.ClientShares[0].Group)
	}
	if !reflect.DeepEqual([]byte{0x41}, ks.ClientShares[0].KeyExchange) {
		t.Errorf("expected %v, got %v", []byte{0x41}, ks.ClientShares[0].KeyExchange)
	}
	if elliptic.Curve(0x11ff) != ks.ClientShares[1].Group {
		t.Errorf("expected %v, got %v", elliptic.Curve(0x11ff), ks.ClientShares[1].Group)
	}
	if !reflect.DeepEqual([]byte{0x42}, ks.ClientShares[1].KeyExchange) {
		t.Errorf("expected %v, got %v", []byte{0x42}, ks.ClientShares[1].KeyExchange)
	}
	if elliptic.P384 != ks.ClientShares[2].Group {
		t.Errorf("expected %v, got %v", elliptic.P384, ks.ClientShares[2].Group)
	}
	if !reflect.DeepEqual([]byte{0x43}, ks.ClientShares[2].KeyExchange) {
		t.Errorf("expected %v, got %v", []byte{0x43}, ks.ClientShares[2].KeyExchange)
	}

	// zero length keyshare vector should throw error
	rawZero := []byte{
		0x0, 0x33, // extension type
		0x0, 0x7, // extension length
		0x0, 0x5, // vec length
		0x0, 0x1d, // X25519
		0x0, 0x0, // group length
		0x42,
	}

	err = ks.Unmarshal(rawZero)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}

	// sending duplicate valid groups should error
	rawDup := []byte{
		0x0, 0x33, // extension type
		0x0, 0x11, // extension length
		0x0, 0xf, // vec length
		0x0, 0x1d, // X25519
		0x0, 0x1, // group length
		0x41,
		0x0, 0x1d, // Non-supported group
		0x0, 0x1, // group length
		0x42,
		0x0, 0x18, // P-384
		0x0, 0x1, // group length
		0x43,
	}

	err = ks.Unmarshal(rawDup)
	if !errors.Is(err, dtlserrors.ErrDuplicateKeyShare) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrDuplicateKeyShare, err)
	}
}

func TestKeyShare_Unmarshal_ClientHello_TruncatedEntries(t *testing.T) {
	// bad ext: vecLen=1, then a single byte to force group uint16 read error.
	typeValue := KeyShare{}.TypeValue()
	raw := []byte{
		byte(uint16(typeValue) >> 8), // G115: high byte extraction from uint16 extension type in test fixture.
		byte(uint16(typeValue)),      // G115: low byte extraction from uint16 extension type in test fixture.
		0x00, 0x03,                   // ext len = 3
		0x00, 0x01, // vecLen = 1
		0xFF, // only 1 byte, not enough for group uint16
	}

	var ks KeyShare
	err := ks.Unmarshal(raw)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}

	// bad ext 2: one full group but key_exchange length=1 with no bytes
	raw2 := []byte{
		byte(uint16(typeValue) >> 8), // G115: high byte extraction from uint16 extension type in test fixture.
		byte(uint16(typeValue)),      // G115: low byte extraction from uint16 extension type in test fixture.
		0x00, 0x06,                   // ext len = 6 (2 vecLen + 4 bytes below)
		0x00, 0x04, // vecLen = 4
		0x00, 0x1D, // group x25519
		0x00, 0x01, // key len = 1 (but 0 bytes present) -> read fails
	}

	err = ks.Unmarshal(raw2)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}
}

func TestKeyShare_Unmarshal_HelloRetryRequest(t *testing.T) {
	typeValue := KeyShare{}.TypeValue()
	raw := []byte{
		0x0, 0x33, // extension type
		0x0, 0x2, // extension length
		0x0, 0x1d, // X25519
	}

	var ks KeyShare
	err := ks.Unmarshal(raw)
	if err != nil {
		t.Error(err)
	}
	if ks.SelectedGroup == nil {
		t.Error("expected non-nil")
	}
	if elliptic.X25519 != *ks.SelectedGroup {
		t.Errorf("expected %v, got %v", elliptic.X25519, *ks.SelectedGroup)
	}
	if ks.ServerShare != nil {
		t.Errorf("expected nil, got %v", ks.ServerShare)
	}
	if ks.ClientShares != nil {
		t.Errorf("expected nil, got %v", ks.ClientShares)
	}

	// Unsupported group in HelloRetryRequest
	raw2 := []byte{
		0x0, 0x33, // extension type
		0x0, 0x2, // extension length
		0x0, 0x1, // unsupported group
	}

	err = ks.Unmarshal(raw2)
	if err != nil {
		t.Error(err)
	}
	if ks.SelectedGroup != nil {
		t.Errorf("expected nil, got %v", ks.SelectedGroup)
	}

	ks.TypeValue()
	rawTrunc := []byte{
		byte(uint16(typeValue) >> 8), // G115: high byte extraction from uint16 extension type in test fixture.
		byte(uint16(typeValue)),      // G115: low byte extraction from uint16 extension type in test fixture.
		0x00, 0x01, 0x00,
	}
	err = ks.Unmarshal(rawTrunc)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}
}

func TestKeyShare_Unmarshal_ServerHello(t *testing.T) {
	typeValue := KeyShare{}.TypeValue()

	raw := []byte{
		0x0, 0x33, // extension type
		0x0, 0x06, // extension length
		0x0, 0x1d, // X25519
		0x0, 0x2, // group length
		0x41, 0x42,
	}

	var ks KeyShare
	err := ks.Unmarshal(raw)
	if err != nil {
		t.Error(err)
	}
	if ks.SelectedGroup != nil {
		t.Errorf("expected nil, got %v", ks.SelectedGroup)
	}
	if ks.ClientShares != nil {
		t.Errorf("expected nil, got %v", ks.ClientShares)
	}
	if ks.ServerShare == nil {
		t.Error("expected non-nil")
	}
	if elliptic.X25519 != ks.ServerShare.Group {
		t.Errorf("expected %v, got %v", elliptic.X25519, ks.ServerShare.Group)
	}
	if !reflect.DeepEqual([]byte{0x41, 0x42}, ks.ServerShare.KeyExchange) {
		t.Errorf("expected %v, got %v", []byte{0x41, 0x42}, ks.ServerShare.KeyExchange)
	}

	// Trailing extra bytes after key_share
	rawTrailing := []byte{
		0x0, 0x33, // extension type
		0x0, 0x08, // extension length
		0x0, 0x1d, // X25519
		0x0, 0x2, // group length
		0x41, 0x42,
		0x43, 0x44,
	}

	err = ks.Unmarshal(rawTrailing)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}

	// bad key length (claims 1, provides 0)
	rawBadLen := []byte{
		byte(uint16(typeValue) >> 8), // G115: high byte extraction from uint16 extension type in test fixture.
		byte(uint16(typeValue)),      // G115: low byte extraction from uint16 extension type in test fixture.
		0x00, 0x04,                   // ext len matches the 4 bytes below
		0x00, 0x1D, // group x25519
		0x00, 0x01, // key len = 1, but 0 bytes provided -> format error
	}
	err = ks.Unmarshal(rawBadLen)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}
}

func TestKeyShare_Unmarshal_Errors(t *testing.T) {
	// wrong extension type
	var w cryptobyte.Builder
	w.AddUint16(0xFFFF)
	w.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) { b.AddUint8(0) })
	rawWrong, _ := w.Bytes()
	var ks KeyShare
	err := ks.Unmarshal(rawWrong)
	if !errors.Is(err, dtlserrors.ErrInvalidExtensionType) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidExtensionType, err)
	}

	// buffer too small (no length field)
	var small cryptobyte.Builder
	small.AddUint16(uint16(KeyShare{}.TypeValue()))

	rawSmall, _ := small.Bytes()
	err = ks.Unmarshal(rawSmall)
	if !errors.Is(err, dtlserrors.ErrBufferTooSmall) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrBufferTooSmall, err)
	}

	// empty extData
	var empty cryptobyte.Builder
	empty.AddUint16(uint16(KeyShare{}.TypeValue()))
	empty.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {})

	rawEmpty, _ := empty.Bytes()
	err = ks.Unmarshal(rawEmpty)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}

	rawTrailing := []byte{
		0x0, 0x33, // extension type
		0x0, 0x8, // extension length
		0x0, 0x5, // vec length
		0x0, 0x1d, // X25519
		0x0, 0x01, // length
		0x42,
		0x43, // trailing byte
	}

	err = ks.Unmarshal(rawTrailing)
	if !errors.Is(err, dtlserrors.ErrInvalidKeyShareFormat) {
		t.Errorf("expected error %v, got %v", dtlserrors.ErrInvalidKeyShareFormat, err)
	}
}

func Test_hasTooManyContexts(t *testing.T) {
	if hasTooManyContexts(false, false, false) {
		t.Error("expected false")
	}
	if hasTooManyContexts(true, false, false) {
		t.Error("expected false")
	} // exactly one
	if hasTooManyContexts(false, true, false) {
		t.Error("expected false")
	}
	if hasTooManyContexts(false, false, true) {
		t.Error("expected false")
	}
	if !hasTooManyContexts(true, true, false) {
		t.Error("expected true")
	} // two
	if !hasTooManyContexts(true, false, true) {
		t.Error("expected true")
	}
	if !hasTooManyContexts(false, true, true) {
		t.Error("expected true")
	}
	if !hasTooManyContexts(true, true, true) {
		t.Error("expected true")
	} // three
}

func FuzzKeyShareUnmarshal(f *testing.F) {
	testCases := [][]byte{
		{
			0x0, 0x33, // extension type
			0x0, 0x11, // extension length
			0x0, 0xf, // vec length
			0x0, 0x1d, // X25519
			0x0, 0x1, // group length
			0x41,
			0x11, 0xff, // Non-supported group
			0x0, 0x1, // group length
			0x42,
			0x0, 0x18, // P-384
			0x0, 0x1, // group length
			0x43,
		},
		{
			0x0, 0x33, // extension type
			0x0, 0x24, // extension length
			0x0, 0x1d, // X25519
			0x0, 0x20, // length 32
			0xa8, 0xf1, 0xb7, 0x2d, 0x70, 0x7e, 0x58, 0xa4, 0x41, 0x73, 0x9e, 0x21,
			0x7b, 0x62, 0x1e, 0xd1, 0x4d, 0x11, 0x69, 0xa6, 0xbf, 0x72, 0x21, 0xec,
			0xaf, 0x76, 0xf3, 0x4e, 0xec, 0x48, 0x52, 0x1,
		},
	}
	for _, tc := range testCases {
		f.Add(tc)
	}

	f.Fuzz(func(t *testing.T, data []byte) {
		ks := KeyShare{}
		err := ks.Unmarshal(data)
		if err != nil {
			return
		}
		hasClientShares := ks.ClientShares != nil
		hasServerShare := ks.ServerShare != nil
		hasHelloRetryRequest := ks.SelectedGroup != nil
		if false != hasTooManyContexts(hasClientShares, hasServerShare, hasHelloRetryRequest) {
			t.Errorf("expected %v, got %v", false, hasTooManyContexts(hasClientShares, hasServerShare, hasHelloRetryRequest))
		}
		// We do not check trailing bytes as non-supported groups are removed.
		testExtDataLength(t, &ks, data, false)
	})
}
