// SPDX-FileCopyrightText: 2026 The Pion community <https://pion.ly>
// SPDX-License-Identifier: MIT

package extension

import (
	"reflect"
	"testing"
)

func TestCookieExt(t *testing.T) {
	extension := CookieExt{Cookie: []byte{0x1, 0x42}}

	raw, err := extension.Marshal()
	if err != nil {
		t.Error(err)
	}

	expect := []byte{
		0x00, 0x2c, // extension type
		0x00, 0x04, // extension length
		0x00, 0x02, // vec length
		0x01, 0x42, // cookie
	}
	if !reflect.DeepEqual(expect, raw) {
		t.Errorf("expected %v, got %v", expect, raw)
	}

	newExtension := CookieExt{}

	if newExtension.Unmarshal(expect) != nil {
		t.Error(newExtension.Unmarshal(expect))
	}
	if !reflect.DeepEqual(extension.Cookie, newExtension.Cookie) {
		t.Errorf("expected %v, got %v", extension.Cookie, newExtension.Cookie)
	}
}

func FuzzCookieExtUnmarshal(f *testing.F) {
	bigCookie := make([]byte, 0xffff)
	bigCookie[0] = 0x00
	bigCookie[1] = 0x2c
	bigCookie[2] = 0xff
	bigCookie[3] = 0xff
	bigCookie[4] = 0xff
	bigCookie[5] = 0xfd

	testcases := [][]byte{
		{
			0x00, 0x2c, // extension type
			0x00, 0x04, // extension length
			0x00, 0x02, // vec length
			0x01, 0x42, // cookie
		},
		{
			0x00, 0x2c, // extension type
			0x00, 0x04, // extension length
			0x00, 0x01, // vec length
			0x01, // cookie
		},
		bigCookie,
	}

	for _, tc := range testcases {
		f.Add(tc)
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		cookieExt := CookieExt{}
		err := cookieExt.Unmarshal(data)
		if err != nil {
			return
		}
		length := len(cookieExt.Cookie)
		if length == 0 {
			t.Errorf("expected non-zero")
		}
		if !(length <= 0xfffd) {
			t.Errorf("expected %v <= %v", length, 0xfffd)
		}
		testExtDataLength(t, &cookieExt, data, true)
	})
}
