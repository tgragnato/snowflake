...
var (
	...
	errALPNNoAppProto              = &protocol.FatalError{Err: errors.New("no application protocol")}
	errBufferTooSmall              = &protocol.TemporaryError{Err: errors.New("buffer is too small")}
	errInvalidExtensionType        = &protocol.FatalError{Err: errors.New("invalid extension type")}
	errInvalidSNIFormat            = &protocol.FatalError{Err: errors.New("invalid server name format")}
	errInvalidCIDFormat            = &protocol.FatalError{Err: errors.New("invalid connection ID format")}
	errLengthMismatch              = &protocol.InternalError{Err: errors.New("data length and declared length do not match")}
	errMasterKeyIdentifierTooLarge = &protocol.FatalError{Err: errors.New("master key identifier is over 255 bytes")}
)
